...
export class AuthService {
  ...
  private async getAccessToken(): Promise<string> {
    ...
    const secret = process.env.WX_APP_SECRET;
    if (!appId || !secret) {
      throw new Error('WX_APP_ID 或 WX_APP_SECRET 未配置');
    }
  ...
  async setupPrivacySetting() {
    ...
    const postData = JSON.stringify({
      ...
      setting_list: [
        { privacy_key: 'UserInfo', privacy_text: '设置用户头像和昵称' },
        { privacy_key: 'Album', privacy_text: '选择头像图片' },
      ],
    });
      ...
      const req = https.request(
        {
          ...
          path: `/wxa/setprivacysetting?access_token=${accessToken}`,
          method: 'POST',
          headers: {
        ...
        (res) => {
          ...
          res.on('end', () => {
            try {
              const result = JSON.parse(data);
              console.log('setPrivacySetting response:', result);
              if (result.errcode === 0) {
                resolve({ success: true, message: '隐私协议配置成功' });
              } else {
                resolve({ success: false, message: `配置失败: ${result.errmsg}`, errcode: result.errcode });
              }
            } catch (e) {
  ...
  async getPrivacySetting() {
    const accessToken = await this.getAccessToken();
    const postData = JSON.stringify({ privacy_ver: 2 });

    return new Promise<any>((resolve) => {
      const req = https.request(
        {
          hostname: 'api.weixin.qq.com',
          path: `/wxa/getprivacysetting?access_token=${accessToken}`,
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Content-Length': Buffer.byteLength(postData),
        ...
        (res) => {
          ...
          res.on('end', () => {
            try {
              const result = JSON.parse(data);
              console.log('getPrivacySetting response:', result);
